...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for verification in sspv:
            # The expiration date should not be higher than 365 days past the `updated_at` field, so only update those entries
            if verification.expiration_date > (verification.updated_at + timedelta(days=settings.VERIFY_STUDENT["DAYS_GOOD_FOR"])):
    ...
    def bulk_update(self, verification_ids):
        ...
        verification_qs.update(expiration_date=F('updated_at') + timedelta(
